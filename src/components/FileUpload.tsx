...
export function FileUpload({ type }: FileUploadProps) {
  ...
  const runPreCheck = useAppStore(s => s.runPreCheck)
  const pendingFile = useAppStore(s => s.preCheckPendingFile)
  const clearPreCheckResult = useAppStore(s => s.clearPreCheckResult)
  ...
  useEffect(() => {
    if (pendingFile && pendingFile.type === type && !showPreCheck) {
      setShowPreCheck(true)
    }
  }, [pendingFile, type, showPreCheck])
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      await runPreCheck(type, text, file.name)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <>
      ...
      <PreCheckDialog open={showPreCheck && pendingFile?.type === type} onClose={handleClosePreCheck} />
    </>
  )
}
